    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                pred_res = predict_results.predictions
                predict_res = np.where(pred_res != -100, pred_res, tokenizer.pad_token_id)
                
                predictions = tokenizer.batch_decode(
                  predict_res  , skip_special_tokens=True, clean_up_tokenization_spaces=True
                )
                ...
                # breakpoint()
                if "wanglab" in model_args.model_name_or_path: 
