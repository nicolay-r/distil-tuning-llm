...
def main():
    ...
    if model_args.model_type == 'adapter':
        ...
        peft_model_id = model_args.checkpoint_dir
    ...
    if training_args.do_predict:
        ...
        val_inputs = []
        predict_results = []
        for val_data in predict_dataset:
            # breakpoint()
            inputs_with_prefix = f"{prefix}{val_data['input']}"
            ...
            val_input = tokenizer(
                ...
                padding=padding, 
                truncation=True, 
                return_tensors="pt").to(training_args.device)
            # breakpoint()
            with torch.no_grad():
                outputs = model.generate(input_ids=val_input["input_ids"], max_new_tokens=10)
            ...
            print(preds)
            predict_results.append(preds[0])
        ...
        # predictions = tokenizer.batch_decode(res_predictions, skip_special_tokens=True, clean_up_tokenization_spaces=True)
