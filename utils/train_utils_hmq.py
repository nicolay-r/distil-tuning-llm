...
from utils.trainer_utils_hmq import TaskPrefixDataCollator, TaskPrefixTrainer
...
def set_wandb(trainer_kwargs, args):
    ...
    wandb.init(group="lmflow", project="MeDistill", name=f"fine-tuning-{args.addi_info}-{dt_object}",
...
def train_and_evaluate(args, run, tokenizer, tokenized_datasets, compute_metrics):
    ...
    if args.model_type == 'peft':
        ...
        model = T5ForConditionalGeneration.from_pretrained(
            args.from_pretrained)  # args.from_pretrained通常是一个字符串，指向预训练模型的存储位置，可以是本地路径或者在线模型库的标识符
