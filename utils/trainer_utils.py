...
class TaskPrefix_COS(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss = self.alpha * pred_outputs.loss + (1. - self.alpha) * cos_loss
...
class TaskPrefixTrainerShareEncoder(Seq2SeqTrainer):
    def __init__(self, alpha, data_collator=None,**kwargs):
        super().__init__(**kwargs) # 调用了当前类的父类（或超类）的 __init__ 方法。
        # breakpoint()
        self.alpha = alpha
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        encoder_loss = slef._cosine_similarity(pred_outputs.encoder_hidden_states, expl_outputs.encoder_hidden_states)
        # head_model = T5WithMLPHead(expl_outputs, model, mlp_hidden_dim).to(device)
        # head_model = T5WithMLPHead(model, mlp_hidden_dim).to(device)
        # breakpoint()
        # expl_logits, expl_loss = self.head_model(**inputs['expl'])
        ...
        # loss = self.alpha * pred_outputs.loss + (1. - self.alpha) * expl_outputs.loss
        loss = self.alpha * pred_outputs.loss + (1. - self.alpha) * encoder_loss
    ...
    def _cosine_similarity(hidden_state1, hidden_state2):
        """
        计算两个隐藏状态之间的余弦相似度。

        参数:
        hidden_state1 (torch.Tensor): 第一个隐藏状态，形状为 (batch_size, hidden_size)
        hidden_state2 (torch.Tensor): 第二个隐藏状态，形状为 (batch_size, hidden_size)

        返回:
        torch.Tensor: 每个批次样本的余弦相似度，形状为 (batch_size,)
        """
        # 计算余弦相似度
        cos_sim = F.cosine_similarity(hidden_state1, hidden_state2, dim=1)
        return cos_sim
...
class DynamicLossTrainer(Seq2SeqTrainer):
    def __init__(self, alpha, data_collator=None,**kwargs):
        super().__init__(**kwargs) # 调用了当前类的父类（或超类）的 __init__ 方法。
        self.alpha = alpha
        # self.output_rationale = output_rationale
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if current_epoch < 3: #前3个epoch都用一样的比例训练
            self.alpha = 0.5
        else:
            self.alpha = (current_epoch-5.)/10. + self.alpha 
            # breakpoint()
            if self.alpha > 1:
                self.alpha = 0.9
            else:
